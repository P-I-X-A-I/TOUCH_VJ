...
@implementation moduleBasis

@synthesize moduleName;
@synthesize moduleIcon;
@synthesize Num_Of_Pages;
@synthesize segmentPageTitle_ARRAY;
@synthesize viewForSegment_ARRAY;
...
- (void)becomeCurrent
{
    NSLog(@"become Current %@", moduleName);
}

- (void)becomeBackground
{NSLog(@"become Background %@", moduleName);}

- (void)setCurrentSlotIndex:(Byte)index
{
    CURRENT_SLOT = index;
    NSLog(@"%@ set current slot index %d", moduleName, CURRENT_SLOT);
    [self initGUI];
}
...
- (void)readShaderSourcePath:(NSString*)path vs:(GLuint*)vsObj fs:(GLuint*)fsObj pg:(GLuint*)pgObj
{
    ...
    NSString* VSstring = [[shaderSource componentsSeparatedByString:@"%"] objectAtIndex:0];
    NSString* FSstring = [[shaderSource componentsSeparatedByString:@"%"] objectAtIndex:1];
    
    
    const GLchar* vs_source = (GLchar*)[VSstring UTF8String];
    const GLchar* fs_source = (GLchar*)[FSstring UTF8String];
    ...
    *fsObj = glCreateShader(GL_FRAGMENT_SHADER);
    ...
    NSLog(@"%@ VS compile status %d", moduleName, status);
    
    glGetShaderiv(*fsObj, GL_COMPILE_STATUS, &status);
    NSLog(@"%@ FS compile status %d", moduleName, status);
    
    *pgObj = glCreateProgram();
    
    glAttachShader( *pgObj, *vsObj);
    glAttachShader( *pgObj, *fsObj);
...
- (void)linkProgram:(GLuint*)pgObj
{
    ...
    glGetProgramiv(*pgObj, GL_INFO_LOG_LENGTH, &logLength);
    if( logLength > 0 )
    {
        GLchar* log = (GLchar*)malloc(logLength);
        glGetProgramInfoLog(*pgObj, logLength, &logLength, log);
        NSLog(@"\n\n %@ link log : \n%s\n\n", moduleName, log );
        free(log);
    }
    
    glGetProgramiv(*pgObj, GL_LINK_STATUS, &status);
    NSLog(@"%@ link status %d", moduleName, status );
}
...
- (void)validateProgramAndDeleteShader_vs:(GLuint*)vsObj fs:(GLuint*)fsObj pg:(GLuint*)pgObj
{
    ...
    glGetProgramiv(*pgObj, GL_VALIDATE_STATUS, &status);
    NSLog(@"%@ validate status %d", moduleName, status);
...
{
    //NSLog(@"testModule isTapped %f %f", *tapX_ptr, *tapY_ptr);
}

- (void)isLongPressed_X:(GLfloat*)LPress_X_ptr Y:(GLfloat*)LPress_Y_ptr index:(Byte)LPress_INDEX num:(Byte)NUM
{
    NSLog(@"testModule isLongpressed %1.3f %1.3f %d", *LPress_X_ptr, *LPress_Y_ptr, LPress_INDEX);
}

- (void)isPanned_Value:(GLfloat*)Value_ptr num:(Byte)NUM
{
    GLfloat* ptr = Value_ptr;
    GLfloat posX = *ptr;    ptr++;
    GLfloat posY = *ptr;    ptr++;
    GLfloat transX = *ptr;  ptr++;
    GLfloat transY = *ptr;  ptr++;
    GLfloat vecX = *ptr;    ptr++;
    GLfloat vecY = *ptr;    ptr++;
    GLfloat velocity = *ptr;
    
    NSLog(@"x:%1.3f y:%1.3f tx:%1.3f ty:%1.3f vx:%1.3f vy:%1.3f vel:%1.3f", posX, posY, transX, transY, vecX, vecY, velocity);
...
- (void)isPinched_Center:(GLfloat*)Center_ptr Radius:(GLfloat*)Radius_ptr Scale:(GLfloat*)Scale_ptr Velocity:(GLfloat*)Vel_ptr num:(Byte)NUM
{
    ...
    GLfloat center_x = *Cptr;   Cptr++;
    GLfloat center_y = *Cptr;
    GLfloat radius = *Radius_ptr;
    GLfloat scale = *Scale_ptr;
    GLfloat velocity = *Vel_ptr;

    NSLog(@"cx:%1.3f cy:%1.3f radius:%1.3f scale:%1.3f vel:%1.3f", center_x, center_y, radius, scale, velocity );
}
...
- (void)dealloc
{
    ...
    // segment title release
    for( i = 0 ; i < [segmentPageTitle_ARRAY count] ; i++ )
    {
        [[segmentPageTitle_ARRAY objectAtIndex:i] release];
    }
    
    // segment array release
    [segmentPageTitle_ARRAY release];
    
    
    // view array release
    [viewForSegment_ARRAY release];
